    ...
    where TItem : class, IModelId
{
    ...
    [Parameter]
    public bool WithItemsProvider { get; set; } = false;
    ...
    {
        if (_renderWithVirtualize != Virtualize)
        {
            _renderWithVirtualize = Virtualize;
            renderingStopwatch.Reset();
            renderingStopwatch.Start();
