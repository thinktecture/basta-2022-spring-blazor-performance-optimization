...
public abstract class CollectionControllerBase<T> : Controller
{
    ...
    [HttpGet]
    [HttpHead]
    public async Task<IActionResult> GetCollectionAsync([FromQuery] string? searchTerm, [FromQuery] int skip = 0, [FromQuery] int take = 1000, CancellationToken cancellationToken = default)
    {
